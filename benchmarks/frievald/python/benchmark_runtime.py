...
def parse_int_list(csv: str) -> List[int]:
    """Return list of ints from comma-separated values, ignoring empty items."""
    return [int(x) for x in csv.split(',') if x.strip()]


def time_function(func, *args) -> float:
    """Measure wall-clock seconds for the supplied callable."""
...
def benchmark_sample(n: int, frievald_iters: List[int], skip_triple_above: int, rng: np.random.Generator, strassen_threshold: int, sample_id: int):
    """Collect timing records for every algorithm at the provided matrix size ``n``."""
    records = []
    A = generate_random_matrix(n, rng=rng)
    B = generate_random_matrix(n, rng=rng)
    t_np = time_function(matmul_numpy, A, B)
    C = matmul_numpy(A, B)
    # NumPy run provides both a timing baseline and a ground-truth product for Frievald.
...
def run_runtime_benchmark(args: argparse.Namespace):
    """Execute the runtime benchmark across randomly sampled matrix sizes."""
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    rng = np.random.default_rng(args.seed)
    all_records = []
    for sample_id in range(args.samples):
        n = rng.integers(args.n_min, args.n_max + 1)
        # Capture every algorithm timing for this sample before moving on.
    ...
    if args.append and RESULTS_PATH.exists():
        # Append mode keeps prior samples so long runs can be chunked safely.
        df.to_csv(RESULTS_PATH, mode='a', header=False, index=False)
        print(f"Appended sampled runtime benchmark to {RESULTS_PATH.resolve()} ({len(df)} rows)")
    else:
        # Default behavior overwrites for a clean slate.
        df.to_csv(RESULTS_PATH, index=False)
        print(f"Saved sampled runtime benchmark to {RESULTS_PATH.resolve()} ({len(df)} rows)")


def build_parser():
    """Create the CLI argument parser used by the runtime benchmark."""
