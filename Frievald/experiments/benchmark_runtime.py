...
from src.algorithms import frievald_verify, matmul_numpy, matmul_triple_loop, matmul_strassen
...
def build_parser():
    ...
    p.add_argument("--samples", type=int, default=500, help="Number of random size samples")
    p.add_argument("--n-min", type=int, default=32, help="Minimum matrix size (inclusive)")
    p.add_argument("--n-max", type=int, default=2000, help="Maximum matrix size (inclusive)")
    ...
    p.add_argument("--skip-triple-loop-above", type=int, default=200, help="Skip naive triple loop above this size (record NaN)")
