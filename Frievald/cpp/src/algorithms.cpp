...
Matrix strassen_recursive(const Matrix& A, const Matrix& B, int threshold) {
    ...
    // C22 = M1 - M2 + M3 + M6
    Matrix C22 = add(sub(sub(M1, M2), M3), M6); // Wait, formula check: M1 + M3 - M2 + M6

    // Correction on C22 formula:
    // Wikipedia: C22 = M1 - M2 + M3 + M6
    // Let's re-verify standard Strassen formulas:
    // M1 = (A11+A22)(B11+B22)
    // M2 = (A21+A22)B11
    // M3 = A11(B12-B22)
    // M4 = A22(B21-B11)
    ...
    // My code above: add(sub(sub(M1, M2), M3), M6) -> (M1 - M2) - M3 + M6. This is WRONG.
    // Should be M1 - M2 + M3 + M6.
...
bool frievald_verify(const Matrix& A, const Matrix& B, const Matrix& C, int k, std::mt19937& rng, bool force_iterations) {
    size_t n = A.rows();
    ...
    for (int iter = 0; iter < k; ++iter) {
        ...
        // Check if ABr == Cr
        for (size_t i = 0; i < n; ++i) {
            if (std::abs(ABr(i, 0) - Cr(i, 0)) > 1e-9) {
                if (!force_iterations) {
                    return false;
                }
                all_correct = false;
